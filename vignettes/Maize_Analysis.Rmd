---
title: "cropDiffusionR: How Maize Farmers in the US Southwest Grew and Prospered under El Niño but Suffered under La Niña"
...
# Get county shapefiles for SWUS to add as an additional column to MDB.
SWUS_counties <-
  sf::st_as_sf(
    maps::map(
      "county",
...
# Load location data for each sample. Site location are not saved in this repository, as we want to ensure that archaeological site locations are protected.
sf::sf_use_s2(FALSE)
...
MDB <- cropDiffusionR::maizeDB %>%
  dplyr::left_join(.,
                   readr::read_csv(
                     stringr::str_remove(
                       here::here(
                         "maize_database/Maize_Database_Locations_DONT_DISTRIBUTE.csv"
                       ),
                       "cropDiffusionR/"
                     ),
...
# If you do not have your own location information, then you can run the code below using the coarse scale location data provided by the p3k14c package. If so, then you can uncomment this code and run instead of lines 52-60 above. However, some of the samples may not be available in the p3k14c package, and locations could vary by at least 10 km from the actual site. p3k14c_data has a column that details the location accuracy. Please refer to Table 2 in https://doi.org/10.1038/s41597-022-01118-7 for more information on the location accuracy codes.
# MDB <- cropDiffusionR::maizeDB %>%
#   dplyr::left_join(
#     .,
...
```{r Figure_1, fig.cap="Figure 1. Low-frequency July temperature anomaly reconstruction for the southwestern United States as defined in inset (based on Gillreath-Brown et al. 2024:Figure 5). The extent of the SWUS inset is approximately: xmin = -112.82, xmax = -105.52, ymin = 32.66, ymax = 38.90. Blue line fitted by loess (local polynomial regression fitting) smoothing (α = 0.15). Negative values on the y-axis indicate temperatures cooler than the 1961–1990 average; the horizontal line at 0 marks the average July temperature from 1961¬–1990. Negative values on x-axis indicate years BC.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.width = 15.51, fig.height = 10.32, fig.align = "left"}

# Create a bounding box for limits of the Gillreath-Brown et al. 2024 study.
bb <-
  c(
    "xmin" = -112.81446,
    "xmax" = -105.52416,
...
paleomat_extent <-
  ...
  scale_color_manual(
    ...
    labels = c("Gillreath-Brown et al. 2024 \n Study Area"),
    name = "Data Extent"
  ) +
...
# Plot low-frequency temperature anomaly reconstruction.
anom_plot <- temp_anom %>%
  ggplot2::ggplot(aes(x = year, y = anom)) +
  geom_ribbon(aes(ymax = anom + ci, ymin = anom - ci), alpha = 0.2) +
  ...
  scale_y_continuous(
    limits = c(-2.0, 3.0),
    breaks = seq(-2.0, 3.0, 0.5),
    minor_breaks = seq(-2.0, 3.0, by = 0.25),
    guide = "axis_minor"
  ) +
  theme_bw() +
  xlab("Years BC/AD") +
...
temp_anom_standard <-
  ...
  dplyr::mutate(Series = "Gillreath-Brown et al. 2024") %>%
...
Moberg_paleomat_fig <- Moberg_paleomat %>%
  ...
  geom_line(size = 2.0) +
  scale_color_manual(
    values = c("#A60F2D", "cornflowerblue"),
    labels = c("Gillreath-Brown et al. 2024", "Moberg et al. 2005")
  ) +
  scale_linetype_manual(
...
```{r Figure_3, fig.cap="Figure 3. The ENSO index is calculated as the difference between the temperature reconstruction by Gillreath-Brown et al. (2024) and the low-frequency component of the Moberg et al. (2005) temperature reconstruction for the Northern Hemisphere from AD 133 to 1900. This index is negative when the Gillreath-Brown et al. reconstruction is cooler than that of Moberg et al. (2005), suggesting El Nino-like conditions prevailed in the SWUS. This index is positive when the Gillreath-Brown et al. (2024) reconstruction is warmer than that of Moberg et al. (2005), suggesting La Niña-like conditions. The ENSO Index is computed from the z-scores for the Gillreath-Brown et al. (2024) and the Moberg et al. (2005) reconstructions as displayed in Figure 2.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.width = 17.25, fig.height = 11.5, fig.align = "left"}

# In this vignette, the temperature anomaly is drawn from the 100 year intervals.
temp_anom_standard2 <-
  temp_anom_standard %>%
  dplyr::mutate(update = (anom - mean(anom)) / sd(anom)) %>%
  dplyr::select(-Series)

# For the figure in the publication, we draw on the annual version of the Gillreath-Brown et al. 2024 temperature reconstruction to fit with the Moberg et al. 2005 reconstruction, which also has annual.
# temp_anom_standard2 <-
#   readr::read_csv(
#     "../Paper 1/supp_table_s2_full.csv",
#     show_col_types = FALSE
#   ) %>%
#   dplyr::mutate(Year = as.integer(Year)) %>%
#   dplyr::filter(Year >= 133 & Year <= 1900) %>%
#   dplyr::select(year = Year, anom = 2) %>%
#   dplyr::mutate(update = (anom - mean(anom)) / sd(anom))
...
Moberg_paleomat_diff <-
  temp_anom_standard2 %>%
  dplyr::select(-anom) %>%
  dplyr::left_join(., Moberg_standard2 %>% dplyr::select(-anom), by = "year") %>%
  dplyr::mutate(anom = update.x - update.y) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(anom3 = dplyr::case_when(
    update.y > update.x ~ "Lower",
    update.y < update.x ~ "Upper",
    TRUE ~ NA
  ))

# Moberg LF and paleomat
Moberg_paleomat_diff_fig <- Moberg_paleomat_diff %>%
  ggplot2::ggplot(aes(x = year, y = anom)) +
  geom_hline(
    yintercept = 0,
    color = "darkgrey",
...
```{r Figure_5, fig.cap="Figure 5. Earliest site with maize in each of the labeled political subdivisions, using data from the Ancient Maize Map (Blake et al. 2017) with additions listed in Supplemental Table S1. The linear model shows the regression of latitude on date for the labeled sites. The slope of the regression line implies an average spread rate of 0.001 degrees/year for this sample (r2 = 0.01; p > F = 0.72).", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.width = 17.25, fig.height = 11.5, fig.align = "left"}
...
# Write output to file, but without location data.
# tmp4 <- MDB_min_fig4 %>%
#   dplyr::select(-c(Long, Lat))
# readr::write_rds(tmp4, here::here("vignettes/output/earliest_maize_province.rds"))
...
```{r Figure_6, fig.cap="Figure 6. Location of maize’s northern frontier through time. Shown is the earliest site with maize in each century, with the constraint that only sites further north than earlier sites appear in the plot. Data from the Ancient Maize Map (Blake et al. 2017) with additions listed in Supplemental Table S1. The linear model shows the regression of latitude on date for the labeled sites. The slope of the regression line implies an average spread rate of 0.003 degrees/year for this sample (r2 = 0.76; p > F < 0.001). Between just Guilà Naquitz and Las Capas however the implied rate of spread is ~8.7 times more rapid (0.026 degrees/year).", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.width = 17.25, fig.height = 11.5, fig.align = "left"}
...
# tmp5 <- MDB_min_fig5 %>%
#   dplyr::select(-c(Long, Lat))
# readr::write_rds(tmp5, here::here("vignettes/output/maize_frontier_mexico.rds"))
...
# tmp6 <- MDB_min_fig6 %>%
#   dplyr::select(-c(Long, Lat))
# readr::write_rds(tmp6, here::here("vignettes/output/maize_frontier_swus.rds"))
...
NASW_gdd_data <-
  raster::as.data.frame(cropDiffusionR::WorldClim_annual_gdd, xy = TRUE)
...
figure4 <-
  ...
  theme_bw() +
  geom_sf(
    data = cropDiffusionR::swus_mexico_states,
    color = "#2b2b2b",
    fill = "transparent",
    size = 0.4,
    inherit.aes = FALSE
...
# Statistics for Figures 5–7

```{r stats_table, echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'asis'}

stats_table <-
  purrr::map2_dfr(
    list(.x = MDB_min_fig4, MDB_min_fig5, MDB_min_fig6),
    .y = list("Figure 5", "Figure 6", "Figure 7"),
    .f = function(x, y) {
      lm(Lat ~ Date, data = x) %>%
        summary() %>%
        {
...
knitr::kable(stats_table,
             caption = "Statistics Table. Summary of statistics for the linear models in Figure 5 through 7.")
