...
MDB_full_cal <- read.csv("../Paper 2/Maize_Database_Calibrated.csv", stringsAsFactors = FALSE)
...
# Change all text that says "NA to NA" from the rcarbon output to a regular NA.
MDB_full_cal <- MDB_full_cal %>%
  dplyr::mutate(dplyr::across(everything(), ~ gsub("NA to NA", NA, .x)))
...
MDB_full_cal_min <- MDB_full_cal %>%
  ...
  dplyr::relocate(MinBP, .after = MedianBP) %>%
  dplyr::filter(str_detect(Location_name, "^Mexico") | str_detect(State_prov, "Arizona|Mexico|Utah|Colorado")) %>%
...
MDB_full_cal_min %>%
  ...
  scale_x_continuous(breaks = seq(-4500, 2000, 500),
                     minor_breaks = seq(-4500, 2000, 250),
                     limits = c(-4500, 2000)) +
  scale_y_continuous(trans = "reverse",
                     breaks = seq(500, 5500, 500),
                     limits = c(5500, 500),
                     minor_breaks = seq(500, 5500, 250),
                     guide = "axis_minor") +
  xlab("Years BC/AD") +
  ylab("Annual Accumulated Growing Degree Days") +
  #ggpubr::color_palette(palette = "jco") +
...
ggplot2::ggsave("vignettes/figures/maize_frontier_gdd_timeBins.png", dpi = 600)
