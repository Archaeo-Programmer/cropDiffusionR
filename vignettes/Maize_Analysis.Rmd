...
#   dplyr::left_join(., ENSOMaizeImpacts::maizeDB, by = "LabID")
...
usa_mexico_states2 <- usa_mexico_states %>% 
  # MDB_min is the data from Figure 3
  dplyr::mutate(state_name = ifelse(state_name %in% c(MDB_min$Province, "Mexico"), state_name, NA_character_))
...
ggplot2::ggsave(figure4, filename = "vignettes/figures/Figure_4_new.png", width = 17.25, height = 11.5, dpi = 600)
...
mtplot <- 
  ...
  ggplot(aes(x = date, y = Lat, color = anom)) +
  geom_point(size = 2) +
...
mtplot2 <- 
  MDB_min_fig4 %>%
  dplyr::filter(Province %in% c("Arizona", "New Mexico", "Utah", "Colorado")) %>% 
  dplyr::left_join(., temp_anom, by = c("date" = "year")) %>%
  dplyr::mutate(Province = as.factor(Province)) %>%
  dplyr::arrange(date) %>%
  dplyr::mutate(labels = paste0(
...
figure5 <-
  MDB_min_fig5 %>%
  dplyr::mutate(Province = fct_reorder(Province, Date, .fun = 'min')) %>%
  dplyr::arrange(Date) %>%
...
ggplot2::ggsave(
  figure5,
  filename = "vignettes/figures/Figure_5_new.png",
  width = 17.25,
  height = 11.5,
  dpi = 600
)
...
ggplot2::ggsave(
  figure6,
  filename = "vignettes/figures/Figure_6_new.png",
  width = 17.25,
  height = 11.5,
  dpi = 600
)
...
garb3 %>%
  #arrange(desc(Lat)) %>%
  dplyr::mutate(Province = as.factor(Province)) %>%
  ggplot(., aes(x = date, y = Lat, color = fct_rev(Province))) +
  ...
  geom_point(shape = 19, size = 4, aes(color = fct_rev(Province))) +
...
garb3 %>%
  dplyr::mutate(Province = as.factor(Province),
                date = as.numeric(date),
                gdd = as.double(gdd)) %>%
  ggplot(., aes(x = date, y = gdd, color = fct_rev(Province))) +
