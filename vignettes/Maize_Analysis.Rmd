...
#   filter(str_detect(Location_name, "^Mexico") | str_detect(State_prov, "Arizona|Mexico|Utah|Colorado"))
# 
# readr::write_csv(MDB_full_cal, "../Paper 2/Maize_Database_Calibrated_SW_ONLY.csv")

MDB_full_cal <- readr::read_csv("../Paper 2/Maize_Database_Calibrated_SW_ONLY.csv", col_types = cols())
...
plot.with.inset <-
  ggdraw() +
  draw_plot(anom_plot) +
  draw_plot(paleomat_extent, x = 0.04, y = .638, width = .51, height = .34)


ggsave(filename = "vignettes/figures/paleomat_with_studyArea.png", 
...
```{r process_maize_frontier_gdd_century, message = FALSE, warning = FALSE, results = 'hide'}
...
# Change all text that says "NA to NA" from the rcarbon output to a regular NA.
MDB_full_cal <- MDB_full_cal %>%
  dplyr::mutate(dplyr::across(NA_select, ~ stringr::str_replace_all(.x, "NA to NA", NA_character_))) %>% 
  #dplyr::filter(stringr::str_detect(Location_name, "^Mexico") | stringr::str_detect(State_prov, "Arizona|New Mexico|Utah|Colorado"))
  dplyr::filter(stringr::str_detect(State_prov, "Arizona|New Mexico|Utah|Colorado"))
...
MDB_full_cal_min <- MDB_full_cal %>%
  ...
  dplyr::mutate(bin = cut(
    MinBP,
    breaks = seq(plyr::round_any(min(MinBP), 100, f = floor), plyr::round_any(max(MinBP), 100, f = ceiling), by = 100),
    include.lowest = TRUE
  )) %>% 
  dplyr::group_by(bin) %>%
  ...
  # dplyr::mutate(date = rcarbon::BPtoBCAD(MinBP)) %>%
  # dplyr::select(Lab_code, Site_name, date, bin, Latitude, gdd, everything())
  #dplyr::mutate(gdd = as.double(Latitude)) %>% 
  dplyr::filter(Latitude == cummax(Latitude)) %>%
  #dplyr::filter(row_number() %in% c(1, 8:10, 12, 36:37, 39:41)) %>% 
  dplyr::mutate(date = rcarbon::BPtoBCAD(MinBP)) %>% 
...
#   ungroup() %>% 
#   dplyr::arrange(desc(MinBP)) %>% 
#   dplyr::mutate(gdd = as.double(gdd)) %>% 
#   mutate(next.smaller.day = row_number()[find.next.smaller(1, gdd)], 
...
```{r maize_frontier_gdd_century_fig, fig.cap="Figure.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.height = 7.73, fig.align = "left"}

lm1 <- lm(Latitude ~ date, data = MDB_full_cal_min)

plot(MDB_full_cal_min$date, MDB_full_cal_min$Latitude,
...
MDB_full_cal_min %>%
  ...
  dplyr::arrange(date) %>%
  #dplyr::mutate(labels = paste0(str_replace(Site_name, " \\s*\\([^\\)]+\\)", ""), ", \n ", State_prov)) %>% 
  ...
  # )  +
  ggrepel::geom_label_repel(
    aes(colour = State_prov, label = Site_name),
    #aes(label = labels),
    max.iter = 4500,
    cex = 6.7,
    show.legend  = F,
  ...
  #   minor_breaks = seq(-4000, 1500, 100),
  #   limits = c(-4100, 1500),
  #   guide = "axis_minor"
  # ) +
  scale_y_continuous(
  ...
  # scale_y_continuous(trans = "reverse",
  #                    breaks = seq(500, 6500, 500),
  #                    limits = c(6500, 500),
  #                    minor_breaks = seq(500, 6500, 250),
  #                    guide = "axis_minor") +
  ...
  ggpubr::color_palette(palette = "jco") +
  theme_bw()  +
  guides(color = guide_legend(override.aes = list(fill = NA), title="States", direction = "horizontal")) +
  theme(
    #legend.justification = 'left',
    legend.position = "bottom",
    legend.title = element_text(size = 19, family = "Helvetica", face = "bold"),
    #legend.title = element_blank(),
    legend.direction = "vertical",
    legend.text = element_text(size = 18, colour = "black"),
...
ggplot2::ggsave(output, filename = "vignettes/figures/maize_frontier_latitude_SW_ONLY_timeBins.png", width = 17.25, height = 11.5, dpi = 600)
...
ggplot2::ggsave(output, filename = "vignettes/figures/earliest_sites_by_state_province.png", width = 17.25, height = 11.5, dpi = 600)
