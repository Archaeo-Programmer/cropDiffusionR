...
MDB <- cropDiffusionR::maizeDB %>%
  dplyr::left_join(
    ...
    readr::read_csv(stringr::str_remove(here::here(
      "maize_database/Maize_Database_Locations_DONT_DISTRIBUTE.csv"), "cropDiffusionR/"),
  ...
  dplyr::mutate(county_st = ifelse(!is.na(REGION) & !is.na(NAME), paste(NAME, REGION, sep = ", "), NA)) %>% 
  sf::st_drop_geometry()

# If you do not have your own location information, then you can run the code below using the mostly coarse scale location data provided by the p3k14c package. If so, then you can uncomment this code and run instead of lines 52-60 above. However, some of the samples may not be available in the p3k14c package, and locations could vary at least 10 km from the actual site. p3k14c_data has a column that details the location accuracy. Please refer to Table 2 in https://doi.org/10.1038/s41597-022-01118-7 for more information on the location accuracy codes.
# MDB <- cropDiffusionR::maizeDB %>%
#   dplyr::left_join(
#     .,
...
MDB_gdd <- MDB %>%
  ...
  dplyr::select(LabID) %>%
  cropDiffusionR::extract_GDD(gdd_raster = cropDiffusionR::WorldClim_annual_gdd,  sites = .)
...
Moberg_paleomat_fig <- Moberg_paleomat %>%
  ...
  ggplot2::ggplot(aes(x = year, y = update, colour = Series, linetype = Series)) +
  geom_hline(
    yintercept = 0,
    color = "darkgrey",
    lty = 3,
