...
NA_select <- colSums("NA to NA" == MDB_full_cal %>% dplyr::select(-geometry)) > 0
...
# 
# ok3 <- ok3 %>% 
#   dplyr::mutate(ID = sample(100:3000, 45, replace=F)) %>% 
#   dplyr::select(ID, abc = MinBP, value = gdd)
# 
# 
# # Reorder the gdd to be in descending order.
# garb <- MDB_full_cal_min %>%
...
MDB_full_cal_min %>%
  ...
  scale_x_continuous(
    breaks = seq(-5000, 1500, 500),
    minor_breaks = seq(-5000, 1500, 100),
    limits = c(-5100, 1500),
    guide = "axis_minor"
  ) +
  scale_y_continuous(
...
ggplot2::ggsave("vignettes/figures/maize_frontier_latitude_SW_ONLY_timeBins.png", dpi = 600)
