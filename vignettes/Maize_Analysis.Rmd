...
# 
rcarbon_output <-
  rcarbon::calibrate(x = ok3$Conventional_14C_age_BP, # normalized age
                     errors = ok3$plus_minus_1_sigma_yrs, # one sigma error
                     ids = ok3$Lab_code) # unique ID for each
...
ok3 <-
  readr::read_csv("../Paper 2/processing_data/Utah_maize_dates_updated.csv",
  ...
  colorado <- p3k14c::p3k14c_data %>%
    dplyr::filter(Province == "Colorado") %>%
    dplyr::filter(str_detect(Taxa, "(?i)Zea mays|Maize|Zea maize") & Material != "CHARCOAL") %>% 
    dplyr::filter(!LabID %in% readr::read_csv("../Paper 2/processing_data/Utah_maize_dates_updated.csv",
                  col_types = readr::cols())[,1][[1]])
  write.csv(colorado, "../Paper 2/processing_data/colorado_maize.csv", row.names = F)
