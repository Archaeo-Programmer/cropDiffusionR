...
imgs <- list.files(stringr::str_remove(here::here("Paper 2/animation/"), "cropDiffusionR/"), full.names = TRUE)
...
magick::image_write(image = img_animated,
            path = stringr::str_remove(here::here("Paper 2/animation/maize_anomaly_plots_arranged.gif"), "cropDiffusionR/"))
...
maize_through_time_gdd2_2 <-
  purrr::map2(NASW_sites2_cumulative[7:59], NASW_sites2_cumulative_names[7:59], function(x, y) {
    ...
    ok2 <- ggplot2::ggplot(data = x2, aes(x = Long, y = Lat)) +
      # geom_raster(data = NASW_gdd_data_transformed, aes(x = long, y = lat, fill = GDD),
      #   alpha = 0.8,
      #   na.rm = TRUE
      # ) +
      # scale_fill_gradientn(colors = colorRampPalette(rev(RColorBrewer::brewer.pal(11, "RdBu")))(255),
      ...
      geom_sf(
       ...
       geom_text(aes(label=counter)) +
        geom_text(aes(label=SiteName, x=Inf, y=y_values, hjust=0)) +
      #geom_text(data = x, aes(label = counter, x = Long, y = Lat, colour = x$SiteName), color = "white") +
       geom_segment(
        data = df_seg,
        aes(
          x = long1,
...
#Gif animation
# list file names and read in
imgs2 <- list.files(stringr::str_remove(here::here("Paper 2/animation2/"), "cropDiffusionR/"), full.names = TRUE)
...
magick::image_write(image = img_animated2, 
            path = stringr::str_remove(here::here("Paper 2/animation2/maize_anomaly_plots_arranged_cumulative_1fps.gif"), "cropDiffusionR/"))
...
rayshader::render_snapshot(clear = TRUE, filename = here::here("vignettes/figures/rayshader_gdd.png"))
