...
# Load the temperature anomaly data from paleoMAT output.
temp_anom <- read_csv("https://raw.githubusercontent.com/Archaeo-Programmer/paleoMAT/master/vignettes/tables/supp_table_s2.csv")

MDB_full_cal <- read.csv("../Paper 2/Maize_Database_Calibrated.csv")
...
ggplot2::ggsave("vignettes/figures/maize_frontier_fig.png", dpi = 600)
...
maize_plot <- ggplot(MDB_FULL, aes(x = date, y = Latitude)) + 
  ...
  scale_x_continuous(
    ...
    minor_breaks = seq((-1000 + 100), (1400 - 100), 200)) +
...
t1 <- temp_anom %>% 
  ...
  dplyr::rename(date = 1) %>% 
  dplyr::filter(date >= -1000 & date <= 1600)
t2 <- MDB_FULL %>% 
...
ggplot(multi.plot, mapping = aes(x = date, y = value)) +
  ...
  scale_x_continuous(
    ...
    minor_breaks = seq((-1000 + 100), (1600 -100), 200)
  ) +
