...
dplyr::filter(if ("Guila Naquitz" %in% SiteName) Lat >= MDB %>% dplyr::filter(SiteName == "Guila Naquitz") %>% dplyr::pull(Lat) %>% dplyr::first() else TRUE)
...
# ned_SW <- raster::projectRaster(ok2, ok3, method = 'ngb')
#
...
invisible(
  mapply(
    ggsave,
    file = paste0(
      "/Users/andrewgillreath-brown/Dropbox/WSU/SKOPEII/model/Paper 2/animation/",
      purrr::map_chr(
...
imgs <- list.files("/Users/andrewgillreath-brown/Dropbox/WSU/SKOPEII/model/Paper 2/animation/", full.names = TRUE)
...
magick::image_write(image = img_animated,
            path = "/Users/andrewgillreath-brown/Dropbox/WSU/SKOPEII/model/Paper 2/maize_anomaly_plots_arranged.gif")



# Do another animation but accumulate the data over time.
NASW_sites2_cumulative <- dplyr::bind_rows(NASW_sites2) %>% 
  dplyr::complete(period = seq(-4300, 1900, 100)) %>% 
  dplyr::fill(., c(LabID:Long), .direction = "down") %>% 
  dplyr::arrange(period) %>% 
  dplyr::group_by(period) %>% 
  dplyr::distinct(geometry, .keep_all = TRUE) %>% 
  split(.$period) %>% 
  purrr::accumulate(NASW_sites2, function(a, b) {
    bind_rows(a, b)
  }) %>% 
  purrr::map(., function(x) x %>% dplyr::distinct(geometry, .keep_all = TRUE))
...
maize_through_time_gdd2 <- purrr::map2(NASW_sites2_cumulative, NASW_sites2_cumulative_names, function(x, y)
  ggplot2::ggplot(data = NASW_gdd_data_transformed) +
    geom_raster(aes(x = long, y = lat, fill = GDD),
      alpha = 0.8,
      na.rm = TRUE
...
invisible(
  mapply(
    ggsave,
    file = paste0(
      "/Users/andrewgillreath-brown/Dropbox/WSU/SKOPEII/model/Paper 2/animation2/",
      purrr::map_chr(
...
#Gif animation
# list file names and read in
imgs2 <- list.files("/Users/andrewgillreath-brown/Dropbox/WSU/SKOPEII/model/Paper 2/animation/", full.names = TRUE)
...
# animate at 4 frames per second
img_animated2 <- magick::image_animate(img_joined2, fps = 4)
...
magick::image_write(image = img_animated2,
            path = "/Users/andrewgillreath-brown/Dropbox/WSU/SKOPEII/model/Paper 2/maize_anomaly_plots_arranged_cumulative.gif")
